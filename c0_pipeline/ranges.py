...
def fetch_range(budget,url,start,end,total,dest,etag=None):
 ...
 if not etag or etag.startswith('W/') or not (etag.startswith(chr(34)) and etag.endswith(chr(34))):raise Stop('STRONG_ETAG_REQUIRED')
 if not 0<=start<=end<total or length>8*1024**2:raise Stop('RANGE_OUTSIDE_LIMIT')
 for e in reversed(budget.events()):
  if e.get('url')==url and e.get('range_start')==start and e.get('range_end')==end and e.get('access_result')=='VERIFIED_RANGE' and e.get('resource_total_bytes')==total and e.get('etag')==etag and dest.exists() and dest.stat().st_size==length and sha(dest)==e['sha256']:
   return e
 if dest.exists():raise Stop('EXISTING_RANGE_REQUIRES_REVIEW')
 ...
 try:
  ...
  with r:
   event.update(http_status=r.code,etag=r.headers.get('ETag'),content_range=r.headers.get('Content-Range'),declared_bytes=r.headers.get('Content-Length'),last_modified=r.headers.get('Last-Modified'))
   ...
   if r.headers.get('Content-Encoding','identity')!='identity':raise Stop('RANGE_CONTENT_ENCODING_UNSUPPORTED')
   if r.headers.get('Content-Length') is not None and int(r.headers['Content-Length'])!=length:raise Stop('RANGE_LENGTH_MISMATCH')
...
class RangeView(io.RawIOBase):
 ...
 def __init__(self,total,segments):
  super().__init__();self.total=total;self.segments=sorted(segments,key=lambda x:x[0]);self.pos=0
  if total<0:raise Stop('INVALID_VIRTUAL_SIZE')
  for i,(start,data) in enumerate(self.segments):
   if start<0 or start+len(data)>total:raise Stop('SEGMENT_OUTSIDE_RESOURCE')
   for other,content in self.segments[:i]:
    left=max(start,other);right=min(start+len(data),other+len(content))
    if left<right and data[left-start:right-start]!=content[left-other:right-other]:raise Stop('CONFLICTING_RANGE_BYTES')
 ...
 def read(self,size=-1):
  ...
  end=min(self.pos+size,self.total);out=[];cursor=self.pos
  while cursor<end:
   hits=[(start,data) for start,data in self.segments if start<=cursor<start+len(data)]
   if not hits:raise Stop('UNFETCHED_RANGE_READ')
   start,data=hits[-1];n=min(end-cursor,start+len(data)-cursor)
   out.append(data[cursor-start:cursor-start+n]);cursor+=n
  self.pos=cursor
  return b''.join(out)
 def readinto(self,b):
