#!/usr/bin/env python3
"""CLI for the bounded OC-3 PHOTSYS authority probe."""
...
from oc3lib.galaxy_eligibility_photsys_authority_probe import (
    FAILED, PHOTSYSProbeError, STAGE_ID, dry_run, persist_failure_terminal,
    probe_resource_contract, validate_inputs,
)


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        prog="oc3_galaxy_eligibility_photsys_authority_probe.py",
        description="Validate or run the separately authorized bounded PHOTSYS authority probe.",
    )
...
def argv_sha256(argv: list[str]) -> str:
    complete = ["oc3/.venv/bin/python",
                "oc3/oc3_galaxy_eligibility_photsys_authority_probe.py", *argv]
    ...
    try:
        if args.validate_inputs:
            if args.candidate or args.authorization or args.output_directory:
                raise PHOTSYSProbeError("OFFLINE_ARGUMENT_SCOPE_VIOLATION")
            ...
            if not args.candidate or not args.authorization or not args.output_directory:
                raise PHOTSYSProbeError("FINAL_AUTHORIZATION_REQUIRED")
            result = probe_resource_contract(
                args.candidate, args.authorization, argv_sha256(supplied), args.output_directory)
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    except PHOTSYSProbeError as exc:
        if args.probe_resource_contract and args.output_directory:
            try:
