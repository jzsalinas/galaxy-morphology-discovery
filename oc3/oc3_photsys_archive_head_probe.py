#!/usr/bin/env python3
"""CLI for the separately authorized desitarget archive HEAD-only probe."""
...
from oc3lib.photsys_archive_head_probe import (
    AUTHORIZATION_PATH, CANDIDATE_PATH, OUTPUT_ROOT, STAGE_ID, HeadProbeError,
    dry_run, execute, validate_historical_inputs,
)


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="One exact HEAD request for the desitarget commit archive")
    modes = result.add_mutually_exclusive_group(required=True)
    modes.add_argument("--validate-inputs", action="store_true")
    modes.add_argument("--dry-run", action="store_true")
    modes.add_argument("--probe-desitarget-archive-head", action="store_true")
    ...
    try:
        if args.validate_inputs:
            if args.execute_network or args.authorization:
                raise HeadProbeError("HEAD_PREFLIGHT_ARGUMENT_INVALID")
            ...
            if not args.execute_network or args.authorization is None:
                raise HeadProbeError("HEAD_PROBE_FINAL_AUTHORIZATION_REQUIRED")
