#!/usr/bin/env python3
"""Acquire and analyze a prospectively frozen bundle of exact Git blobs."""
...
from oc3lib.photsys_zero_byte_provenance import (  # noqa: E402
    MANDATORY_SOURCE_PATHS, SEARCH_TERMS, analyze_producer_source,
    search_source_tree,
)
...
def validate_candidate(path: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if (not isinstance(resources, list) or len(resources) != len(MANDATORY_SOURCE_PATHS) or
            [item.get("path") for item in resources] != list(MANDATORY_SOURCE_PATHS)):
...
def execute(candidate_path: Path, standing_authorization: Path, permit: Path,
    ...
    try:
        ...
        trace = analyze_producer_source(source_root)
        write_json_immutable(output / "SEARCH_RESULTS.json", sealed(search))
        write_json_immutable(output / "PRODUCER_TRACE.json", sealed(trace))
        write_json_immutable(output / "RESOURCE_RECEIPTS.json", sealed({"resources": receipts}))
        state = candidate["terminal_mapping"]["success"]
        reason = "EXACT_MANDATORY_SOURCE_BLOBS_VALIDATED"
        observed = {"named_product_generator_status": search["named_product_generator_status"],
                    "normal_N_assignment_detected": trace["normal_N_assignment_detected"],
                    "normal_PHOTSYS_S1_dtype_detected": trace["normal_PHOTSYS_S1_dtype_detected"],
                    "normal_S_assignment_detected": trace["normal_S_assignment_detected"],
                    "supplement_randoms_detected": trace["supplement_randoms_detected"],
                    "write_path_candidate_detected": trace["write_path_candidate_detected"],
                    "zeros_true_output_dtype_contains_PHOTSYS": trace["zeros_true_output_dtype_contains_PHOTSYS"],
                    "zeros_true_path_detected": trace["zeros_true_path_detected"]}
    ...
    transition_completed_action(state_path=state_path, candidate_path=candidate_path, permit_path=permit,
        ...
        reason="AUTONOMOUS_EXACT_GIT_BLOB_BUNDLE_COMPLETED")
    return terminal
