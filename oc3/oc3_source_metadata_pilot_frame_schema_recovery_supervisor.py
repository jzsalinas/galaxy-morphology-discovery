#!/usr/bin/env python3
"""Permit-gated supervisor for one bounded pilot-frame schema recovery worker."""
...
from oc3lib.source_metadata_pilot_frame_schema_recovery import (
    ALLOWED_COLUMN_ACCESS_MISMATCH, DIAGNOSTIC_SCHEMA, FRAME_SELECTION_INTEGRITY_FAILURE,
    GLOBAL_IDENTITY_INTEGRITY_FAILURE, IDENTITY_DECODE_FAILURE,
    PHYSICAL_SCHEMA_CONTRACT_MISMATCH, SCOPE, STAGE_ID, STREAM_CAPTURE_CAP,
    WORKER_RUNTIME_FAILURE, validate_frame_payload,
)
...
def _terminal(state: str, candidate: dict[str, object], diagnostic_sha: str,
    ...
    return sealed({"application_body_bytes_read": 0, "counters": counters,
        "diagnostic_sha256": diagnostic_sha, "failure_class": failure_class,
        "scope": candidate["scope"],
        "stage_id": candidate["stage_id"], "state": state})


def _worker_failure_class(child: dict[str, object]) -> str:
    allowed = {PHYSICAL_SCHEMA_CONTRACT_MISMATCH, ALLOWED_COLUMN_ACCESS_MISMATCH,
        IDENTITY_DECODE_FAILURE, GLOBAL_IDENTITY_INTEGRITY_FAILURE,
        FRAME_SELECTION_INTEGRITY_FAILURE, WORKER_RUNTIME_FAILURE}
    try:
        text = child["stderr"]["snippet_utf8_replacement"]
        lines = [line for line in text.splitlines() if line.strip()]
        value = json.loads(lines[-1])
        code = value["error"]
    except Exception:
        return WORKER_RUNTIME_FAILURE
    return code if code in allowed else WORKER_RUNTIME_FAILURE
...
def supervise(candidate: dict[str, object], output: Path) -> dict[str, object]:
    ...
    write_json_immutable(output / "START_INTENT.json", sealed({
        ...
        "schema_version": "OC3_FRAME_SCHEMA_RECOVERY_START_INTENT_001", "stage_id": STAGE_ID,
    ...
    state = "PILOT_FRAME_SCHEMA_WORKER_FAILED"
    failure_class = _worker_failure_class(child)
    ...
    if child["return_code"] == 0:
        try:
            ...
            state, checkpoint = "PILOT_FRAME_SCHEMA_RECOVERED", "FRAME_VALIDATED"
            failure_class = None
        except Exception as exc:
            state = "PILOT_FRAME_SCHEMA_OUTPUT_INVALID"
            validation_error = getattr(exc, "code", type(exc).__name__)
            failure_class = FRAME_SELECTION_INTEGRITY_FAILURE
    ...
    diagnostic = sealed({"child": child, "diagnostic_schema": DIAGNOSTIC_SCHEMA,
        ...
        "validation_error": validation_error, "worker_error_code": failure_class,
    ...
    terminal = _terminal(state, candidate, file_sha256(output / "EXECUTION_DIAGNOSTIC.json"), failure_class)
    write_json_immutable(output / "TERMINAL.json", terminal)
    return terminal


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Supervised offline pilot-frame schema recovery")
    modes = result.add_mutually_exclusive_group(required=True)
    modes.add_argument("--validate-candidate", action="store_true")
    modes.add_argument("--recover-schema-correct-pilot-frame", action="store_true")
    ...
    try:
        ...
        if args.validate_candidate:
            print(json.dumps({"network_requests":0, "real_frame_reads":0,
                              "state":"READY_AT_SCHEMA_CORRECT_SUPERVISED_FRAME_RECOVERY_BOUNDARY"}, sort_keys=True))
            return 0
        ...
        from oc3lib.source_metadata_pilot_frame_schema_recovery_governor import consume_permit, validate_permit
        ...
        return 0 if terminal["state"] == "PILOT_FRAME_SCHEMA_RECOVERED" else 2
    except Exception as exc:
        print(json.dumps({"error":getattr(exc,"code",type(exc).__name__), "network_requests":0,
                          "state":"PILOT_FRAME_SCHEMA_RECOVERY_SUPERVISOR_BLOCKED"}, sort_keys=True), file=sys.stderr)
        return 2
