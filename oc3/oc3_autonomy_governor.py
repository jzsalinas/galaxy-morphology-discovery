#!/usr/bin/env python3
"""Offline CLI for the frozen OC3 autonomy policy/lifecycle engine."""
...
from oc3lib.autonomy_governor import (
    ...
    STANDING_AUTHORIZATION_PATH, AutonomyError, activate_standing_autonomy,
    audit_staged_compact_artifacts, compact_status, enter_stop_requires_human,
...
def parser() -> argparse.ArgumentParser:
    result=argparse.ArgumentParser(description="Deterministic offline OC3 autonomy governor")
    ...
    for name in ("validate","status","dry-run","activate-standing-authorization","register-action",
                 "evaluate","issue-permit","transition-completed-action","audit-staged","enter-stop",
    ...
    result.add_argument("--consumption-directory",type=Path)
...
def _candidate(args) -> Path:
    ...
    if not isinstance(registered,dict): raise AutonomyError("NO_REGISTERED_PENDING_ACTION")
...
def main(argv: list[str]|None=None) -> int:
    ...
    try:
        if args.validate:
            ...
            if args.permit_output is None: raise AutonomyError("AUTONOMOUS_PERMIT_OUTPUT_REQUIRED")
            ...
            if (args.permit is None or args.terminal is None or args.terminal_sha256 is None or
                    args.consumption_directory is None):
                raise AutonomyError("AUTONOMY_TRANSITION_ARGUMENT_REQUIRED")
            output=transition_completed_action(state_path=args.state,candidate_path=_candidate(args),
                ...
                consumption_directory=args.consumption_directory,terminal_path=args.terminal,
            ...
            if args.stop_report is None or args.blocker_code is None:
                raise AutonomyError("AUTONOMY_STOP_ARGUMENT_REQUIRED")
            ...
            if args.outcome is None or args.final_report is None:
                raise AutonomyError("SCIENTIFIC_TERMINAL_ARGUMENT_REQUIRED")
