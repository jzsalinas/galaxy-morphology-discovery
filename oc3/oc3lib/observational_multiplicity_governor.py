...
RUNTIME_TREE_PREFIXES = ("oc3/observational_multiplicity/",)
...
def validate_mandate(path: Path = MANDATE_PATH) -> dict[str, object]:
    ...
    if (set(value) != required or value.get("schema_version") != "OC3_OBSERVATIONAL_MULTIPLICITY_AUTONOMY_MANDATE_003" or
            ...
            value.get("autonomous_action_contract") != "OC3_OBSERVATIONAL_MULTIPLICITY_ACTION_CONTRACT_002" or
    ...
    if value.get("budgets") != {"application_body_bytes_parent":16777216,
        "application_body_bytes_remaining":16777216,"concurrency":1,
        "network_requests_parent":12,"network_requests_remaining":12,
...
def validate_state(path: Path = STATE_PATH) -> dict[str, object]:
    ...
    if (type(value.get("requests_remaining")) is not int or not 0 <= value["requests_remaining"] <= 12 or
            type(value.get("body_budget_remaining")) is not int or not 0 <= value["body_budget_remaining"] <= 16777216):
...
def validate_action_validation_receipt(binding: object, contract: dict[str, object]) -> dict[str, object]:
    ...
    if (set(receipt) != required or receipt.get("schema_version") != "OC3_OBSERVATIONAL_MULTIPLICITY_ACTION_VALIDATION_RECEIPT_002" or
...
def validate_autonomous_action_candidate(path: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if (not isinstance(contract, dict) or set(contract) != required or
            contract.get("schema_version") != "OC3_OBSERVATIONAL_MULTIPLICITY_ACTION_CONTRACT_002" or
...
def _registered_binding(candidate_path: Path, candidate: dict[str, object], contract: dict[str, object], *, first=False) -> dict[str, object]:
    return {"action_contract_schema":"OC3_OBSERVATIONAL_MULTIPLICITY_ACTION_CONTRACT_002","action_kind":contract["action_kind"],
...
def build_permit(*, candidate_path: Path, state_path: Path, standing_authorization_path: Path,
    ...
    return sealed({"action_kind":contract["action_kind"],"authorization_basis":"STANDING_OBSERVATIONAL_MULTIPLICITY_AUTONOMY_MANDATE_001",
        ...
        "schema_version":"OC3_OBSERVATIONAL_MULTIPLICITY_EXECUTION_PERMIT_002","scientific_firewall":contract["scientific_firewall"],
...
def validate_permit(permit_path: Path, *, candidate_path: Path, state_path: Path,
    ...
    if (set(permit) != required or permit.get("schema_version") != "OC3_OBSERVATIONAL_MULTIPLICITY_EXECUTION_PERMIT_002" or
            permit.get("permit_type") != "AUTONOMOUS_EXECUTION_PERMIT" or
            permit.get("authorization_basis") != "STANDING_OBSERVATIONAL_MULTIPLICITY_AUTONOMY_MANDATE_001" or
