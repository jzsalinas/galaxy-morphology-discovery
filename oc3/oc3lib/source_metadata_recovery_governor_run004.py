...
from .source_metadata_recovery_factory_run004 import (
    ACTION_FAMILIES, FIRST_CANDIDATE_RELATIVE, RUN_ID,
    SELF_REPAIR_IMPLEMENTATION_AGGREGATE, binding,
    build_first_candidate, build_next_candidate,
)
from .source_metadata_self_repair import validate_candidate_execution_binding
...
SPEC = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_PRODUCTION_SPEC_005.md"
POLICY_CONTRACT = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_POLICY_CORE_004.md"
RUNBOOK = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUNBOOK_004.md"
PARTITION_AMENDMENT = PROJECT / "OC3_SOURCE_METADATA_GIT_DIFF_RUNTIME_PARTITION_AMENDMENT_001.md"
...
POLICY_MANIFEST = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_POLICY_CORE_MANIFEST_004.json"
MANDATE = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_MANDATE_004.json"
RUN_003_CLOSURE = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_003_CLOSURE_001.json"
INHERITED_TRANSPORT_AUTHORITY = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RUN_004_INHERITED_TRANSPORT_AUTHORITY_001.json"
TRANSPORT_BOOTSTRAP = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RUN_004_TRANSPORT_BOOTSTRAP_001.json"
DRIFT_MATRIX = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RUN_004_DRIFT_MATRIX_001.json"
PRODUCTION_FIRST_CANDIDATE = PROJECT / FIRST_CANDIDATE_RELATIVE
FIRST_CANDIDATE = PRODUCTION_FIRST_CANDIDATE
ACTION_REGISTRY = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_ACTION_REGISTRY_004.json"
TECHNICAL_AUTHORITIES = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_TECHNICAL_AUTHORITIES_002.json"
STATE = PROJECT / "oc3/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STATE_004.json"
STANDING_AUTHORIZATION = PROJECT / "oc3/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STANDING_AUTHORIZATION_004.json"
CANDIDATE_VALIDATION = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_FIRST_CANDIDATE_VALIDATION_004.json"
SELF_REPAIR_AMENDMENT = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_TECHNICAL_SELF_REPAIR_AMENDMENT_001.md"
SELF_REPAIR_REPORT = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_TECHNICAL_SELF_REPAIR_IMPLEMENTATION_REPORT_001.md"
SELF_REPAIR_REPLAY = PROJECT / "oc3/environment_setup/OC3_SOURCE_METADATA_AUTONOMOUS_TECHNICAL_SELF_REPAIR_REPLAY_RECEIPT_001.json"
LEDGER_ROOT = PROJECT / "oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_004_LEDGER"
PERMIT_ROOT = PROJECT / "oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_004_PERMITS"
...
IMPLEMENTATION_FILES = (
    ...
    "oc3/oc3lib/source_metadata_recovery_factory_run004.py",
    "oc3/oc3lib/source_metadata_recovery_governor_run004.py",
    "oc3/oc3lib/source_metadata_recovery_executors_run004.py",
    "oc3/oc3lib/source_metadata_git_change_partition.py",
    "oc3/oc3lib/source_metadata_path_identity.py",
    "oc3/oc3lib/source_metadata_self_repair.py",
    "oc3/oc3_source_metadata_recovery_mission_runner_run004.py",
    "oc3/oc3_source_metadata_recovery_supervisor_run004.py",
    "oc3/oc3_source_metadata_recovery_worker_run004.py",
)
...
def implementation_aggregate() -> str:
    """Frozen validated aggregate for the Run-004 self-repair authority."""
    return SELF_REPAIR_IMPLEMENTATION_AGGREGATE


def control_plane_aggregate() -> str:
...
def inherited_active_adapter_binding() -> dict[str, object]:
    authority=_load(INHERITED_TRANSPORT_AUTHORITY, "INHERITED_TRANSPORT_AUTHORITY_INVALID")
    implementation=PROJECT / "oc3/recovery_adapters/source_metadata/transport_registry.py"
    result={"adapter_id":"query_manager_public_anonymous_v1",
        "implementation_path":"oc3/recovery_adapters/source_metadata/transport_registry.py",
        "implementation_sha256":file_sha256(implementation),
        "inherited_transport_authority":binding(INHERITED_TRANSPORT_AUTHORITY),
        "technical_transport_contract":authority.get("technical_transport_contract"),
        "test_receipts":authority.get("test_receipts")}
    if not all(isinstance(result[name],dict) for name in
            ("inherited_transport_authority","technical_transport_contract","test_receipts")):
        raise RecoveryEnvelopeError("INHERITED_TRANSPORT_AUTHORITY_INVALID")
    return result


def _active_binding_shape_valid(value: object) -> bool:
    if not isinstance(value,dict): return False
    common={"adapter_id","implementation_path","implementation_sha256",
        "technical_transport_contract","test_receipts"}
    inherited=common|{"inherited_transport_authority"}
    repaired=common|{"technical_patch_manifest","git_change_partition"}
    return set(value) in (inherited,repaired)
...
def validate_static_authorities() -> dict[str, dict[str, object]]:
    values = {"invariants": _load(INVARIANTS, "SCIENTIFIC_INVARIANTS_INVALID"),
        ...
        "run_003_closure": _load(RUN_003_CLOSURE, "RECOVERY_PREDECESSOR_RUN_INVALID"),
        "inherited_transport": _load(INHERITED_TRANSPORT_AUTHORITY, "INHERITED_TRANSPORT_AUTHORITY_INVALID"),
        "drift_matrix": _load(DRIFT_MATRIX, "RECOVERY_DRIFT_MATRIX_INVALID"),
    ...
    drift=values["drift_matrix"]
    if (drift.get("schema_version") != "OC3_SOURCE_METADATA_RUN_004_DRIFT_MATRIX_001" or
            drift.get("run_id") != RUN_ID or any(drift.get(name) != 0 for name in
            ("scientific_semantic_drift","observational_contract_drift","provider_resource_drift",
             "rights_drift","resource_scope_drift","budget_drift","acceptance_criteria_drift"))):
        raise RecoveryEnvelopeError("RECOVERY_DRIFT_MATRIX_INVALID")
    registry = values["action_registry"]
    if (registry.get("schema_version") != "OC3_SOURCE_METADATA_RECOVERY_ACTION_REGISTRY_004" or
    ...
    if (policy.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_POLICY_CORE_MANIFEST_004" or
            ...
            policy.get("implementation_aggregate") != SELF_REPAIR_IMPLEMENTATION_AGGREGATE or
            policy.get("control_plane_aggregate") != control_plane_aggregate() or
    ...
    closure = values["run_003_closure"]
    if (closure.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_003_CLOSURE_001" or
            closure.get("run_id") != "OC3-SOURCE-METADATA-AUTONOMOUS-RECOVERY-RUN-003" or
            closure.get("stop_reason") != "TECHNICAL_PATCH_GIT_DIFF_STATE_PATH_BINDING_CONFLICT" or
            closure.get("network_requests") != 2 or closure.get("body_bytes") != 14899 or
            closure.get("scientific_rows_accepted") != 0 or closure.get("source_values_accepted") != 0 or
            closure.get("material_actions") != 0 or closure.get("self_repair_episodes") != 0 or
            any(not isinstance(closure.get(name), dict) for name in
                ("authorization", "terminal_state", "mission_final_report", "binding_conflict",
                 "g01_terminal", "documentary_evidence")) or
            any(binding(PROJECT / closure[name]["path"]) != closure[name] for name in
                ("authorization", "terminal_state", "mission_final_report", "binding_conflict",
                 "g01_terminal", "documentary_evidence"))):
        raise RecoveryEnvelopeError("RECOVERY_PREDECESSOR_RUN_INVALID")
    inherited=values["inherited_transport"]
    expected_conclusions=["BYTE_IDENTICAL_FROZEN_ADQL", "PUBLIC_ANONYMOUS_GET",
        "REDIRECTS_FORBIDDEN", "BODY_CAP_ENFORCED", "CSV_BODY_ACCEPTED_WITH_RECORDED_TEXT_HTML_CONTENT_TYPE"]
    if (inherited.get("schema_version") != "OC3_SOURCE_METADATA_RUN_004_INHERITED_TRANSPORT_AUTHORITY_001" or
            inherited.get("run_id") != RUN_ID or inherited.get("source_run_id") != closure["run_id"] or
            inherited.get("adapter_id") != "query_manager_public_anonymous_v1" or
            inherited.get("technical_conclusions") != expected_conclusions or
            inherited.get("scientific_rows_accepted") != 0 or inherited.get("material_actions") != 0 or
            inherited.get("network_requests_charged_to_run_004") != 0 or
            inherited.get("predecessor_run") != binding(RUN_003_CLOSURE) or
            inherited.get("adapter_implementation") != binding(PROJECT / "oc3/recovery_adapters/source_metadata/transport_registry.py") or
            inherited.get("technical_transport_contract") != binding(PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RUN_004_INHERITED_TRANSPORT_CONTRACT_001.json") or
            inherited.get("test_receipts") != binding(PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RUN_004_INHERITED_TEST_RECEIPTS_001.json") or
            inherited.get("query_semantic_hashes") != {r["id"]:r["semantic_sha256"] for r in values["invariants"]["queries"]} or
            not isinstance(inherited.get("evidence"), list) or not inherited["evidence"] or
            any(binding(PROJECT / row["path"]) != row for row in inherited["evidence"])):
        raise RecoveryEnvelopeError("INHERITED_TRANSPORT_AUTHORITY_INVALID")
    if (mandate.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_MANDATE_004" or
            ...
            mandate.get("candidate_factory") != binding(PROJECT / "oc3/oc3lib/source_metadata_recovery_factory_run004.py") or
            mandate.get("mission_runner") != binding(PROJECT / "oc3/oc3_source_metadata_recovery_mission_runner_run004.py") or
            ...
            mandate.get("predecessor_run") != binding(RUN_003_CLOSURE) or
            mandate.get("inherited_transport_authority") != binding(INHERITED_TRANSPORT_AUTHORITY) or
            mandate.get("drift_matrix") != binding(DRIFT_MATRIX) or
            mandate.get("policy_core_manifest") != binding(POLICY_MANIFEST) or
            mandate.get("implementation_aggregate") != SELF_REPAIR_IMPLEMENTATION_AGGREGATE or
            mandate.get("control_plane_aggregate") != control_plane_aggregate() or
            mandate.get("autonomous_self_repair") != {"authorized_after_activation": True,
                "maximum_episodes_per_mission": 2, "maximum_episodes_per_defect_class": 1,
                "post_repair_rebinding_authorized": True}):
...
def validate_candidate_self_binding(candidate_path: Path,
    ...
    try:
        validate_candidate_execution_binding(candidate=candidate,
            actual_candidate_path=candidate_path, project_root=PROJECT, cwd=PROJECT)
    except Exception as exc:
...
def validate_candidate(path: Path) -> dict[str, object]:
    ...
    required = {"action_kind", "active_adapter", "active_adapter_binding", "action_registry", "application_body_reservation", "authority_classes", "budget_accounting", "candidate_path", "canonical_execution_binding", "command_argv",
        "command_argv_sha256", "expected_terminal_classes", "implementation_aggregate", "material_budget_reservation", "mission_execution_argv", "mission_execution_argv_sha256", "mission_scope",
        "mutable_technical_surface", "network_request_reservation", "output_directory",
        "negative_capabilities", "parent_action_terminal", "permit_path", "policy_core_manifest", "recovery_budget", "recovery_generation",
        "recovery_graph", "remaining_budgets", "request_class", "resume", "retries", "rights_constraints", "run_id",
        ...
        "git_change_partition"}
    if (set(value) != required or value.get("schema_version") != "RECOVERY_ACTION_FACTORY_V4" or
            ...
            value.get("policy_core_manifest") != binding(POLICY_MANIFEST) or
            value.get("budget_accounting") != {"counter_scope": "MISSION_LOCAL_RUN_004",
                "cross_mission_global_cap": None, "historical_requests_are_provenance_only": True} or
            value.get("rights_constraints") != {"access": "PUBLIC_ANONYMOUS_ONLY",
                "credentials_authorized": False, "redistribution_authorized": False} or
            not isinstance(value.get("mission_execution_argv"), list) or
            sha256_bytes(canonical(value["mission_execution_argv"])) != value.get("mission_execution_argv_sha256")):
    ...
    self_repair = value.get("self_repair_authority")
    if (not isinstance(self_repair, dict) or
            self_repair.get("amendment") != binding(SELF_REPAIR_AMENDMENT) or
            self_repair.get("implementation_report") != binding(SELF_REPAIR_REPORT) or
            self_repair.get("replay_receipt") != binding(SELF_REPAIR_REPLAY) or
            self_repair.get("implementation_aggregate") != SELF_REPAIR_IMPLEMENTATION_AGGREGATE or
            self_repair.get("maximum_episodes_per_mission") != 2 or
            self_repair.get("maximum_episodes_per_defect_class") != 1 or
            self_repair.get("post_repair_fresh_artifacts_required") is not True):
        raise RecoveryEnvelopeError("RECOVERY_SELF_REPAIR_AUTHORITY_INVALID")
    expected_mission_command = [str(PROJECT / "oc3/.venv/bin/python"),
        str(PROJECT / "oc3/oc3_source_metadata_recovery_mission_runner_run004.py"),
        "--run-mission", "--state", str(STATE), "--standing-authorization",
        str(STANDING_AUTHORIZATION)]
    if value["mission_execution_argv"] != expected_mission_command:
        raise RecoveryEnvelopeError("RECOVERY_MISSION_EXECUTION_IDENTITY_INVALID")
    budget = _load(RECOVERY_BUDGET, "RECOVERY_BUDGET_INVALID")
    ...
    if (active_required and (not _active_binding_shape_valid(active_binding) or
...
def validate_first_candidate(path: Path = FIRST_CANDIDATE) -> dict[str, object]:
    ...
    if (value["action_kind"] != "MATERIAL_SOURCE_METADATA_ACQUISITION" or value["request_class"] != "MATERIAL" or
            value["trigger_failure_class"] != "TECHNICAL_PATCH_VALIDATED" or
            value["network_request_reservation"] != 5 or value["application_body_reservation"] != 67_108_864 or
            value["material_budget_reservation"] != {"body_bytes": 67_108_864, "requests": 5} or
            value["technical_budget_reservation"] != {"body_bytes": 0, "requests": 0} or
            value["active_adapter_binding"] != inherited_active_adapter_binding()):
    ...
    expected = build_first_candidate(parent_terminal_path=parent, action_registry_path=ACTION_REGISTRY,
        ...
        self_repair_amendment_path=SELF_REPAIR_AMENDMENT,
        self_repair_report_path=SELF_REPAIR_REPORT,
        self_repair_replay_path=SELF_REPAIR_REPLAY,
        inherited_adapter_binding=inherited_active_adapter_binding())
...
def validate_state(path: Path = STATE) -> dict[str, object]:
    ...
    required = {"active", "active_adapter", "active_adapter_binding", "adapter_states", "agentic_repair_request", "body_budget_material_parent", "code_repair_generation", "current_stage",
        "candidate_validation", "first_candidate", "implementation_aggregate", "last_action_terminal", "last_action_terminal_sha256", "last_classification", "material_body_bytes_remaining",
        "material_actions_attributable", "material_requests_remaining", "mission_id", "mission_scope", "network_requests_attributable", "next_action_kind", "permits_issued", "predecessor_run",
        "policy_core_manifest", "recovery_generation", "registered_pending_action", "requests_material_parent", "schema_version", "sealed",
        "run_id", "sequence", "scientific_outcome", "standing_authorization", "state", "stop_reason", "execution_status",
        "self_repair_defect_classes", "self_repair_episodes",
        ...
        "validated_change_partition"}
    if (set(value) != required or value.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STATE_004" or
            value.get("mission_id") != MISSION_ID or value.get("run_id") != RUN_ID or
            value.get("mission_scope") != MISSION_SCOPE or value.get("predecessor_run") != binding(RUN_003_CLOSURE) or
            value.get("first_candidate") != binding(FIRST_CANDIDATE) or
            value.get("policy_core_manifest") != binding(POLICY_MANIFEST) or
            value.get("implementation_aggregate") != SELF_REPAIR_IMPLEMENTATION_AGGREGATE or
            value.get("candidate_validation") != binding(CANDIDATE_VALIDATION) or
            value.get("self_repair_episodes") not in (0, 1, 2) or
            not isinstance(value.get("self_repair_defect_classes"), list) or
            len(value["self_repair_defect_classes"]) != value["self_repair_episodes"] or
            len(set(value["self_repair_defect_classes"])) != len(value["self_repair_defect_classes"])):
    ...
    if value["state"] == STATE_WAITING and (value["technical_requests_remaining"] != 8 or
            value["technical_body_bytes_remaining"] != 2_097_152 or
            value["material_requests_remaining"] != 5 or
            value["material_body_bytes_remaining"] != 67_108_864 or
            value["permits_issued"] != 0 or value["recovery_generation"] != 0 or
            value["self_repair_episodes"] != 0 or value["network_requests_attributable"] != 0 or
            value["material_actions_attributable"] != 0 or value["execution_status"] != "NOT_STARTED"):
        raise RecoveryEnvelopeError("RECOVERY_WAITING_STATE_INVALID")
    ...
    if (not isinstance(value["adapter_states"],dict) or
            ...
            (value["active_adapter"] is not None and
             (value["adapter_states"].get(value["active_adapter"]) != "ACTIVE" or
              not _active_binding_shape_valid(active_binding) or
...
def activate(*, state_path: Path, authorization_path: Path, activated_at_utc: str) -> dict[str, object]:
    ...
    body.update({"active": True, "active_adapter":"query_manager_public_anonymous_v1",
        "active_adapter_binding":inherited_active_adapter_binding(),
        "adapter_states":{**state["adapter_states"],"query_manager_public_anonymous_v1":"ACTIVE"},
        "current_stage": "AWAITING_NEXT_ACTION_REGISTRATION",
        "standing_authorization": binding(authorization_path), "state": STATE_ACTIVE,
        "execution_status": "STARTED", "sequence": state["sequence"] + 1})
...
def issue_permit(*, state_path: Path, candidate_path: Path, output_path: Path,
    ...
    permit = sealed({"application_body_reservation": candidate["application_body_reservation"],
        ...
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_EXECUTION_PERMIT_004", "single_use": True,
...
def validate_permit(*, permit_path: Path, candidate_path: Path,
    ...
    if (set(permit) != required or permit.get("schema_version") != "OC3_AUTONOMOUS_RECOVERY_EXECUTION_PERMIT_004" or
...
def consume_permit(*, permit_path: Path, candidate_path: Path, consumed_at_utc: str) -> Path:
    ...
    write_json_immutable(marker, sealed({"candidate_path": str(candidate_path.resolve().relative_to(PROJECT)),
        ...
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_PERMIT_CONSUMPTION_004"}))
    return marker
...
def create_worker_capability(*, candidate_path: Path, permit_path: Path,
    ...
    capability = sealed({"application_body_reservation": candidate["application_body_reservation"],
        ...
        "request_class": candidate["request_class"], "run_id": RUN_ID,
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_WORKER_CAPABILITY_004", "single_use": True,
...
def validate_worker_capability(*, capability_path: Path, candidate_path: Path,
    ...
    expected_path = str(candidate_path.resolve().relative_to(PROJECT))
    if (set(capability) != required or capability.get("schema_version") != "OC3_AUTONOMOUS_RECOVERY_WORKER_CAPABILITY_004" or
...
def consume_worker_capability(*, capability_path: Path, candidate_path: Path,
    ...
    write_json_immutable(marker, sealed({"candidate_path": str(candidate_path.resolve().relative_to(PROJECT)),
        ...
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_CAPABILITY_CONSUMPTION_004"}))
    return marker


def agentic_paths(generation: int) -> dict[str, Path]:
    return {"request": LEDGER_ROOT / f"AGENTIC_REPAIR_REQUEST_{generation:02d}.json",
        "patch": PROJECT / f"oc3/INPUTS/TECHNICAL_PATCH_MANIFEST_RUN_004_{generation:02d}.json",
        "contract": PROJECT / f"oc3/INPUTS/TECHNICAL_TRANSPORT_CONTRACT_RUN_004_{generation:02d}.json",
        "receipts": PROJECT / f"oc3/INPUTS/TECHNICAL_REPAIR_TEST_RECEIPTS_RUN_004_{generation:02d}.json",
        "partition": LEDGER_ROOT / f"GIT_CHANGE_PARTITION_{generation:02d}.json"}
...
def complete_agentic_handoff(*, state_path: Path) -> dict[str, object]:
    ...
    if not all(paths[k].is_file() for k in ("patch","contract","receipts","partition")):
        return state
    ...
    partition=_load(paths["partition"],"GIT_CHANGE_PARTITION_INVALID")
    mutable_paths=[row["path"] for row in patch.get("changed_paths",[]) if isinstance(row,dict)]
    validate_partition(partition,expected_base=request.get("current_git_head"),
        expected_mutable_paths=mutable_paths)
    requirement=RuntimeStateRequirement(path=str(STATE.relative_to(PROJECT)),run_id=RUN_ID,
        schema_version="OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STATE_004",
        standing_authorization=state["standing_authorization"],
        ledger_prefix="oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_004_LEDGER/")
    expected_partition=classify_git_changes(base_commit=request["current_git_head"],
        mutable_prefixes=request["allowed_path_prefixes"],project_root=PROJECT,
        runtime_requirement=requirement,lifecycle_validator=validate_state)
    ...
    body["validated_change_partition"]=binding(paths["partition"])
...
def transition_action(*, state_path: Path, candidate_path: Path, terminal_path: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    updated["network_requests_attributable"] = int(state["network_requests_attributable"]) + int(
        terminal.get("network_requests_started", 0))
    updated["material_actions_attributable"] = int(state["material_actions_attributable"]) + (
        1 if terminal.get("request_class") == "MATERIAL" else 0)
    ...
    if terminal.get("adapter_activated") is not None:
        ...
        partition=candidate.get("git_change_partition")
        ...
        if (candidate["action_kind"] != "OFFLINE_TECHNICAL_REPAIR" or
                ...
                partition != state.get("validated_change_partition") or
                ...
                terminal.get("git_change_partition") != partition or
        ...
        if updated.get("active") is True:
            ...
            updated["active_adapter_binding"]={**actual_binding,
                ...
                "test_receipts":receipts,"git_change_partition":partition}
        else:
...
def validate_standing_authorization(path: Path, *, state_path: Path,
    ...
    required = {"action_registry", "authorized", "candidate_factory_sha256", "first_candidate", "initial_state_sha256",
        "control_plane_aggregate", "implementation_aggregate", "mandate", "mission_id", "mission_runner_sha256", "mutable_technical_surface", "predecessor_run", "run_id",
        "policy_core_manifest", "recovery_budget", "recovery_graph", "schema_version",
        "scientific_invariants", "sealed", "self_repair_authority", "technical_authorities",
        "inherited_transport_authority"}
    if (set(value) != required or value.get("schema_version") != "OC3_SOURCE_METADATA_RECOVERY_STANDING_AUTHORIZATION_004" or
            ...
            value.get("first_candidate") != binding(FIRST_CANDIDATE) or
            value.get("predecessor_run") != binding(RUN_003_CLOSURE) or
            value.get("inherited_transport_authority") != binding(INHERITED_TRANSPORT_AUTHORITY) or
            value.get("mandate") != binding(MANDATE) or value.get("policy_core_manifest") != binding(POLICY_MANIFEST) or
            value.get("implementation_aggregate") != SELF_REPAIR_IMPLEMENTATION_AGGREGATE or
            value.get("control_plane_aggregate") != control_plane_aggregate() or
            value.get("self_repair_authority") != binding(SELF_REPAIR_AMENDMENT) or
            ...
            value.get("mission_runner_sha256") != file_sha256(PROJECT / "oc3/oc3_source_metadata_recovery_mission_runner_run004.py") or
            value.get("candidate_factory_sha256") != file_sha256(PROJECT / "oc3/oc3lib/source_metadata_recovery_factory_run004.py")):
        raise RecoveryEnvelopeError("RECOVERY_STANDING_AUTHORIZATION_INVALID")
    return value
