"""Closed validator for supervised, offline pilot-frame recovery."""
...
from .source_metadata_pilot_frame_recovery import (
    ACTION_KIND, DIAGNOSTIC_SCHEMA, SCOPE, STAGE_ID, STREAM_CAPTURE_CAP,
)

SPEC = PROJECT / "OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_FIRST_ACTION_SPEC_001.md"
CANDIDATE = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_CANDIDATE_001.json"
RECEIPT = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_CANDIDATE_VALIDATION_001.json"
PERMIT = PROJECT / "oc3/SOURCE_METADATA_PILOT_FRAME_RECOVERY_AUTONOMY_PERMITS/OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_PERMIT_001.json"
AUTHORIZATION = PROJECT / "oc3/OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_STANDING_AUTHORIZATION_001.json"
STATE = PROJECT / "oc3/OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_AUTONOMY_STATE_001.json"
OUTPUT = PROJECT / "oc3/source_metadata_pilot_frame_recovery/OC3-SOURCE-METADATA-PILOT-FRAME-RECOVERY-001"
FRAME = OUTPUT / "PILOT_FRAME.json"
EXECUTABLE = PROJECT / "oc3/.venv/bin/python"
SUPERVISOR = PROJECT / "oc3/oc3_source_metadata_pilot_frame_recovery_supervisor.py"
WORKER = PROJECT / "oc3/oc3_source_metadata_pilot_frame_recovery_worker.py"
IMPLEMENTATION_FILES = (
    "OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_FIRST_ACTION_SPEC_001.md",
    "oc3/oc3_source_metadata_pilot_frame_recovery_supervisor.py",
    "oc3/oc3_source_metadata_pilot_frame_recovery_worker.py",
    "oc3/oc3lib/source_metadata_pilot_frame_recovery.py",
    "oc3/oc3lib/source_metadata_pilot_frame_recovery_validation.py",
)
EXPECTED_INPUTS = (
...
def expected_worker_command() -> list[str]:
    return [str(EXECUTABLE), str(WORKER), "--run-worker", "--candidate", str(CANDIDATE),
            "--output", str(FRAME)]


def expected_supervisor_argv() -> list[str]:
    return [str(EXECUTABLE), str(SUPERVISOR), "--recover-pilot-frame", "--candidate", str(CANDIDATE),
...
def validate_candidate(path: Path = CANDIDATE) -> dict[str, object]:
    ...
    if (value.get("schema_version") != "OC3_SOURCE_METADATA_PILOT_FRAME_RECOVERY_CANDIDATE_001" or
            ...
            value.get("technical_cause") != "UNKNOWN" or value.get("worker_launches") != 1 or
