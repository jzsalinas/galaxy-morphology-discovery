...
MISSION_SCOPE = "CROSS_ID_FORMALISM_RECOVERY_ONLY"
AUTONOMY_BRANCH = "autopilot/cross-id-formalism-recovery"
...
SCIENTIFIC_SPEC_PATH = PROJECT / "OC3_CROSS_ID_FORMALISM_RECOVERY_SPEC_001.md"
SCIENTIFIC_SPEC_SHA256 = "0e6ed937621a4760b129cd22654f5fde00d53106ef4c0c43dba28fd2fbd97b9d"
POLICY_CORE_CONTRACT_PATH = PROJECT / "OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_POLICY_CORE_CONTRACT_001.md"
POLICY_CORE_MANIFEST_PATH = PROJECT / "oc3/INPUTS/OC3_CROSS_ID_FORMALISM_RECOVERY_POLICY_CORE_MANIFEST_001.json"
MANDATE_DOCUMENT_PATH = PROJECT / "OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_MANDATE_001.md"
MANDATE_PATH = PROJECT / "oc3/INPUTS/OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_MANDATE_001.json"
RUNBOOK_PATH = PROJECT / "OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMOUS_RESEARCH_RUNBOOK_001.md"
STANDING_AUTHORIZATION_PATH = PROJECT / "oc3/OC3_CROSS_ID_FORMALISM_RECOVERY_STANDING_AUTHORIZATION_001.json"
AUTHORIZATION_PATH = STANDING_AUTHORIZATION_PATH
STATE_PATH = PROJECT / "oc3/OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_STATE_001.json"
LEDGER_ROOT = PROJECT / "oc3/CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_LEDGER"
CONSUMPTION_ROOT = LEDGER_ROOT / "PERMIT_CONSUMPTION"
PRIOR_FINAL_REPORT_PATH = PROJECT / "OC3_CROSS_OBSERVER_GROUPING_FINAL_REPORT_001.md"
PRIOR_FINAL_REPORT_SHA256 = "417a7e15cd044e2e41575b3fa2b99f23e902c5562a41c5e62fe85fdd08b837a3"
PRIOR_CLAIM_MATRIX_PATH = PROJECT / "oc3/OC3_CROSS_OBSERVER_GROUPING_FINAL_CLAIM_MATRIX_001.json"
PRIOR_CLAIM_MATRIX_SHA256 = "c483939ad407281beeecac818bf767763a7301920b064c0e10ddd4f5b2b4d37f"
PRIOR_TERMINAL_STATE_PATH = PROJECT / "oc3/OC3_CROSS_OBSERVER_GROUPING_AUTONOMY_STATE_002.json"
PRIOR_TERMINAL_STATE_SHA256 = "64cd28e9626f553d8633bab92b6a24283d30c9bb3c113530d243b0ab18304208"
...
AUTHORITY_CLASSES = (
    ...
    "PRIMARY_CROSS_IDENTIFICATION_LITERATURE",
)
FIREWALL_KEYS = (
...
POLICY_CORE_MEMBERS = (
    "OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_POLICY_CORE_CONTRACT_001.md",
    "oc3/oc3_cross_id_formalism_recovery_governor.py",
    "oc3/oc3lib/cross_id_formalism_recovery_governor.py",
)
COMPACT_ARTIFACT_MAX_BYTES = 5 * 1024 * 1024
RUNTIME_TREE_PREFIXES = ("oc3/cross_id_formalism_recovery/",)
...
def validate_policy_core_manifest(path: Path = POLICY_CORE_MANIFEST_PATH) -> dict[str, object]:
    ...
    if (set(value) != {"active_mutation_result", "contract", "files", "schema_version", "sealed"} or
            value.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_POLICY_CORE_MANIFEST_001" or
...
def validate_mandate(path: Path = MANDATE_PATH) -> dict[str, object]:
    ...
    if (set(value) != required or value.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_MANDATE_001" or
            ...
            value.get("autonomous_action_contract") != "OC3_CROSS_ID_FORMALISM_RECOVERY_ACTION_CONTRACT_001" or
            value.get("stop_state") != STOP_REQUIRES_HUMAN):
        raise GovernorError("AUTONOMY_MANDATE_INVALID")
    if value.get("budgets") != {"application_body_bytes_parent":8388608,
        "application_body_bytes_remaining":8388608,"concurrency":1,
        "network_requests_parent":4,"network_requests_remaining":4,
...
def validate_state(path: Path = STATE_PATH) -> dict[str, object]:
    ...
    if (set(value) != required or value.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_STATE_001" or
            ...
            value.get("request_budget_parent") != 4 or value.get("body_budget_parent") != 8388608):
    ...
    if (type(value.get("requests_remaining")) is not int or not 0 <= value["requests_remaining"] <= 4 or
            type(value.get("body_budget_remaining")) is not int or not 0 <= value["body_budget_remaining"] <= 8388608):
...
def validate_standing_authorization(path: Path, *, expected_initial_state_sha256: str,
    ...
    if (set(value) != required or value.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_STANDING_AUTHORIZATION_001" or
...
def validate_action_validation_receipt(binding: object, contract: dict[str, object]) -> dict[str, object]:
    ...
    if (set(receipt) != required or receipt.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_ACTION_VALIDATION_RECEIPT_001" or
...
def validate_autonomous_action_candidate(path: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if (not isinstance(contract, dict) or set(contract) != required or
            contract.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_ACTION_CONTRACT_001" or
...
def _registered_binding(candidate_path: Path, candidate: dict[str, object], contract: dict[str, object], *, first=False) -> dict[str, object]:
    return {"action_contract_schema":"OC3_CROSS_ID_FORMALISM_RECOVERY_ACTION_CONTRACT_001","action_kind":contract["action_kind"],
...
def build_permit(*, candidate_path: Path, state_path: Path, standing_authorization_path: Path,
    ...
    return sealed({"action_kind":contract["action_kind"],"authorization_basis":"STANDING_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_MANDATE_001",
        ...
        "schema_version":"OC3_CROSS_ID_FORMALISM_RECOVERY_EXECUTION_PERMIT_001","scientific_firewall":contract["scientific_firewall"],
...
def validate_permit(permit_path: Path, *, candidate_path: Path, state_path: Path,
    ...
    if (set(permit) != required or permit.get("schema_version") != "OC3_CROSS_ID_FORMALISM_RECOVERY_EXECUTION_PERMIT_001" or
            permit.get("permit_type") != "AUTONOMOUS_EXECUTION_PERMIT" or
            permit.get("authorization_basis") != "STANDING_CROSS_ID_FORMALISM_RECOVERY_AUTONOMY_MANDATE_001" or
