...
from .source_metadata_recovery_factory_run003 import (
...
SPEC = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_PRODUCTION_SPEC_004.md"
POLICY_CONTRACT = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_POLICY_CORE_003.md"
RUNBOOK = PROJECT / "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUNBOOK_003.md"
...
POLICY_MANIFEST = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_POLICY_CORE_MANIFEST_003.json"
MANDATE = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_MANDATE_003.json"
RUN_002_CLOSURE = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_002_CLOSURE_001.json"
PRODUCTION_FIRST_CANDIDATE = PROJECT / FIRST_CANDIDATE_RELATIVE
FIRST_CANDIDATE = PRODUCTION_FIRST_CANDIDATE
ACTION_REGISTRY = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_ACTION_REGISTRY_003.json"
TECHNICAL_AUTHORITIES = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_TECHNICAL_AUTHORITIES_002.json"
STATE = PROJECT / "oc3/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STATE_003.json"
STANDING_AUTHORIZATION = PROJECT / "oc3/OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STANDING_AUTHORIZATION_003.json"
CANDIDATE_VALIDATION = PROJECT / "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_FIRST_CANDIDATE_VALIDATION_003.json"
...
LEDGER_ROOT = PROJECT / "oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_003_LEDGER"
PERMIT_ROOT = PROJECT / "oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_003_PERMITS"
...
IMPLEMENTATION_FILES = (
    ...
    "oc3/oc3lib/source_metadata_recovery_factory_run003.py",
    "oc3/oc3lib/source_metadata_recovery_governor_run003.py",
    "oc3/oc3lib/source_metadata_recovery_executors_run003.py",
    "oc3/oc3lib/source_metadata_path_identity.py",
    "oc3/oc3lib/source_metadata_self_repair.py",
    "oc3/oc3_source_metadata_recovery_mission_runner_run003.py",
    "oc3/oc3_source_metadata_recovery_supervisor_run003.py",
    "oc3/oc3_source_metadata_recovery_worker_run003.py",
)
...
def implementation_aggregate() -> str:
    """Frozen validated aggregate for the Run-003 self-repair authority."""
...
def validate_static_authorities() -> dict[str, dict[str, object]]:
    values = {"invariants": _load(INVARIANTS, "SCIENTIFIC_INVARIANTS_INVALID"),
        ...
        "run_002_closure": _load(RUN_002_CLOSURE, "RECOVERY_PREDECESSOR_RUN_INVALID"),
    ...
    if (registry.get("schema_version") != "OC3_SOURCE_METADATA_RECOVERY_ACTION_REGISTRY_003" or
    ...
    if (policy.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_POLICY_CORE_MANIFEST_003" or
    ...
    closure = values["run_002_closure"]
    if (closure.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_002_CLOSURE_001" or
            closure.get("run_id") != "OC3-SOURCE-METADATA-AUTONOMOUS-RECOVERY-RUN-002" or
            closure.get("authorization_sha256") != "0b727b876091fa9508b50f651457308e22c6fd1024a9c248328e0ed6f0bb3250" or
            closure.get("terminal_state_sha256") != "1c8d54fe366dbbce8af22bfa0b684d9e014c5f052fae7f307005ccbce7587bd4" or
            closure.get("control_plane_contradiction_sha256") != "a5cdaa5d7fd24534ef7a30d661253cf1bb92eba954347d49b8a4fa92db915e87" or
            closure.get("mission_final_report_sha256") != "6d9abc269c8d080262a2e40271c067766a5dc848474edcdbf4e54ac30a08854b" or
            closure.get("g01_terminal_sha256") != "e41bbe3fb709a66051b91caf07a01f60128a9f15946eaff45f9e765049b105b8" or
            closure.get("g01_raw_response_sha256") != "4605a1303c96cf2156446702e857382cf8365553a2ad5db91b6b2b9b6bb7d3ca" or
            closure.get("stop_reason") != "POLICY_CORE_PATH_CANONICALIZATION_REPAIR_REQUIRED" or
            closure.get("network_requests") != 1 or closure.get("body_bytes") != 2124 or
            closure.get("scientific_rows_accepted") != 0 or closure.get("source_values_accepted") != 0 or
            closure.get("material_actions") != 0 or closure.get("adapter_activated") is not False):
        raise RecoveryEnvelopeError("RECOVERY_PREDECESSOR_RUN_INVALID")
    if (mandate.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_MANDATE_003" or
            ...
            mandate.get("candidate_factory") != binding(PROJECT / "oc3/oc3lib/source_metadata_recovery_factory_run003.py") or
            mandate.get("mission_runner") != binding(PROJECT / "oc3/oc3_source_metadata_recovery_mission_runner_run003.py") or
            ...
            mandate.get("predecessor_run") != binding(RUN_002_CLOSURE) or
...
def validate_candidate(path: Path) -> dict[str, object]:
    ...
    if (set(value) != required or value.get("schema_version") != "RECOVERY_ACTION_FACTORY_V4" or
            ...
            value.get("budget_accounting") != {"counter_scope": "MISSION_LOCAL_RUN_003",
...
def validate_first_candidate(path: Path = FIRST_CANDIDATE) -> dict[str, object]:
    ...
    if (value["action_kind"] != "OFFICIAL_SERVICE_DOCUMENTARY_PROBE" or value["request_class"] != "TECHNICAL" or
            value["trigger_failure_class"] != "TECHNICAL_DIAGNOSTIC_CLASSIFIED" or
            value["network_request_reservation"] != 2 or value["application_body_reservation"] != 262_144 or
            value["material_budget_reservation"] != {"body_bytes": 0, "requests": 0} or
            value["technical_budget_reservation"] != {"body_bytes": 262_144, "requests": 2}):
...
def validate_state(path: Path = STATE) -> dict[str, object]:
    ...
    if (set(value) != required or value.get("schema_version") != "OC3_SOURCE_METADATA_AUTONOMOUS_RECOVERY_STATE_003" or
            value.get("mission_id") != MISSION_ID or value.get("run_id") != RUN_ID or
            value.get("mission_scope") != MISSION_SCOPE or value.get("predecessor_run") != binding(RUN_002_CLOSURE) or
...
def issue_permit(*, state_path: Path, candidate_path: Path, output_path: Path,
    ...
    permit = sealed({"application_body_reservation": candidate["application_body_reservation"],
        ...
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_EXECUTION_PERMIT_003", "single_use": True,
...
def validate_permit(*, permit_path: Path, candidate_path: Path,
    ...
    if (set(permit) != required or permit.get("schema_version") != "OC3_AUTONOMOUS_RECOVERY_EXECUTION_PERMIT_003" or
...
def consume_permit(*, permit_path: Path, candidate_path: Path, consumed_at_utc: str) -> Path:
    ...
    write_json_immutable(marker, sealed({"candidate_path": str(candidate_path.resolve().relative_to(PROJECT)),
        ...
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_PERMIT_CONSUMPTION_003"}))
    return marker
...
def create_worker_capability(*, candidate_path: Path, permit_path: Path,
    ...
    capability = sealed({"application_body_reservation": candidate["application_body_reservation"],
        ...
        "request_class": candidate["request_class"], "run_id": RUN_ID,
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_WORKER_CAPABILITY_003", "single_use": True,
...
def validate_worker_capability(*, capability_path: Path, candidate_path: Path,
    ...
    expected_path = str(candidate_path.resolve().relative_to(PROJECT))
    if (set(capability) != required or capability.get("schema_version") != "OC3_AUTONOMOUS_RECOVERY_WORKER_CAPABILITY_003" or
...
def consume_worker_capability(*, capability_path: Path, candidate_path: Path,
    ...
    write_json_immutable(marker, sealed({"candidate_path": str(candidate_path.resolve().relative_to(PROJECT)),
        ...
        "schema_version": "OC3_AUTONOMOUS_RECOVERY_CAPABILITY_CONSUMPTION_003"}))
    return marker


def agentic_paths(generation: int) -> dict[str, Path]:
    return {"request": LEDGER_ROOT / f"AGENTIC_REPAIR_REQUEST_{generation:02d}.json",
        "patch": PROJECT / f"oc3/INPUTS/TECHNICAL_PATCH_MANIFEST_RUN_003_{generation:02d}.json",
        "contract": PROJECT / f"oc3/INPUTS/TECHNICAL_TRANSPORT_CONTRACT_RUN_003_{generation:02d}.json",
        "receipts": PROJECT / f"oc3/INPUTS/TECHNICAL_REPAIR_TEST_RECEIPTS_RUN_003_{generation:02d}.json"}
...
def validate_standing_authorization(path: Path, *, state_path: Path,
    ...
    if (set(value) != required or value.get("schema_version") != "OC3_SOURCE_METADATA_RECOVERY_STANDING_AUTHORIZATION_003" or
            ...
            value.get("first_candidate") != binding(FIRST_CANDIDATE) or
            value.get("predecessor_run") != binding(RUN_002_CLOSURE) or
            ...
            value.get("mission_runner_sha256") != file_sha256(PROJECT / "oc3/oc3_source_metadata_recovery_mission_runner_run003.py") or
            value.get("candidate_factory_sha256") != file_sha256(PROJECT / "oc3/oc3lib/source_metadata_recovery_factory_run003.py")):
        raise RecoveryEnvelopeError("RECOVERY_STANDING_AUTHORIZATION_INVALID")
    return value
