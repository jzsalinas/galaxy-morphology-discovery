"""Deterministic candidate factory for source-metadata recovery RUN-003."""
...
RUN_ID = "OC3-SOURCE-METADATA-AUTONOMOUS-RECOVERY-RUN-003"
FIRST_CANDIDATE_RELATIVE = "oc3/INPUTS/OC3_SOURCE_METADATA_RECOVERY_FIRST_CANDIDATE_003.json"
SELF_REPAIR_IMPLEMENTATION_AGGREGATE = "d6c4680b7e861baffce27dee14a4270c1304b1e3c841eb6c9977502b2f7d0348"
...
def _paths(action_kind: str, generation: int) -> dict[str, str]:
    stem = f"OC3-SOURCE-METADATA-RECOVERY-RUN-003-G{generation:02d}-{action_kind}"
    output = f"oc3/source_metadata_autonomous_recovery_run_003/{stem}"
    return {"stage_id": stem,
        "candidate_path": f"oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_003_LEDGER/CANDIDATES/{stem}.json",
        "output_directory": output,
        "permit_path": f"oc3/SOURCE_METADATA_AUTONOMOUS_RECOVERY_RUN_003_PERMITS/{stem}.json",
...
def _build_candidate(*, state: Mapping[str, object], parent_terminal_path: Path,
    ...
    payload = {"action_kind": action_kind, "active_adapter": adapter,
        ...
        "budget_accounting": {"counter_scope": "MISSION_LOCAL_RUN_003",
    ...
    runner = canonical_path_identity("oc3/oc3_source_metadata_recovery_mission_runner_run003.py",
...
def build_next_candidate(*, state: Mapping[str, object], parent_terminal_path: Path,
    ...
    """Derive a child candidate and its RUN-003 ledger path."""
...
def build_first_candidate(*, parent_terminal_path: Path, action_registry_path: Path,
        ...
        implementation_aggregate: str, policy_manifest_path: Path,
        self_repair_amendment_path: Path, self_repair_report_path: Path,
        self_repair_replay_path: Path) -> dict[str, object]:
    ...
    bootstrap_state = {"active": True, "state": "ACTIVE", "registered_pending_action": None,
        "last_classification": {"decision": "RECOVER_AUTONOMOUSLY",
            "failure_class": "TECHNICAL_DIAGNOSTIC_CLASSIFIED",
            "next_action_kind": "OFFICIAL_SERVICE_DOCUMENTARY_PROBE"},
        "next_action_kind": "OFFICIAL_SERVICE_DOCUMENTARY_PROBE",
    ...
    paths = _paths("OFFICIAL_SERVICE_DOCUMENTARY_PROBE", 1)
