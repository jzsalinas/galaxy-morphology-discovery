...
def support_for_resource(g,h,descriptors,resource,rectangles):
    ...
    return dict(url=resource['url'],etag=resource['etag'],total_bytes=resource['total_bytes'],cached_ranges=cached,
        ...
        try:
            ...
            with contextlib.redirect_stdout(stream),contextlib.redirect_stderr(stream):
                ...
                for job in jobs:
                    ...
                    else:print(job['rank'],job['band'],'CHECKPOINT_REUSED',flush=True)
                    rows.append(row)
