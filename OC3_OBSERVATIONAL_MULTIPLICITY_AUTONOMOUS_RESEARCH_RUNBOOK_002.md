...
3. Stop. A separately reviewed standing authorization must bind Manifest 002, Mandate 002, the exact waiting-state SHA, and Candidate 004 path/SHA.
...
The first executor command is sealed in Candidate 004. It explicitly supplies candidate, Permit 004, standing authorization, autonomy state, and output directory. Before permit validation or consumption, the executor requires literal equality between the runtime Python executable, executing script token, ordered argument vector, and Candidate 004's sealed `command_argv`. Paths are compared exactly as supplied, without normalization or symlink resolution. The executor consumes the permit before one local audit call and never performs the state completion transition.
